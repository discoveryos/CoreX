...
_start:
    ...
    call _start_c        # call Rust _start_c
    "
);
...
// ----- Serial functions -----
unsafe fn serial_rcvd(device: u16) -> bool { inportb(device + 5) & 1 != 0 }
unsafe fn serial_recv(device: u16) -> u8 { while !serial_rcvd(device) {}; inportb(device) }
unsafe fn serial_transmit_empty(device: u16) -> bool { inportb(device + 5) & 0x20 != 0 }
unsafe fn serial_send(device: u16, out: u8) { while !serial_transmit_empty(device) {}; outportb(device, out); }

// ----- String utils -----
fn strlength(s: &str) -> usize { s.len() }
fn atoi(s: &str) -> i32 { s.bytes().fold(0, |acc, b| acc * 10 + (b - b'0') as i32) }
fn reverse(s: &mut [u8]) { s.reverse(); }

fn itoa(mut n: u64, s: &mut [u8]) -> usize {
    let mut i = 0;
    if n == 0 { s[0] = b'0'; return 1; }
    while n > 0 { s[i] = b'0' + (n % 10) as u8; n /= 10; i += 1; }
    s[..i].reverse(); i
}
...
// ----- Start function called by _start -----
#[no_mangle]

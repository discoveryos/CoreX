    ...
    unsafe {
        ...
        if !unlikely_error(uacpi_table_fadt(&mut fadt)) && !fadt.is_null() {
            ...
            if CENTURY_REGISTER != 0 {
                debugf(
                    "[acpi::info] Found century register for RTC: register{0x%lx}\n",
...
pub fn acpi_poweroff() -> SizeT {
    unsafe {
        ...
        if unlikely_error(ret) {
            debugf(
                "[acpi] Couldn't prepare for poweroff: %s\n",
                uacpi_status_to_string(ret),
            );
            return err(EIO);
        }
        ...
        if unlikely_error(ret_poweroff) {
            ...
            debugf(
                "[acpi] Couldn't power off the system: %s\n",
...
pub fn acpi_reboot() -> SizeT {
    unsafe {
        ...
        if unlikely_error(ret) {
            debugf(
                "[acpi] Couldn't restart the system: %s\n",
                uacpi_status_to_string(ret),
            );
            return err(EIO);
        }
